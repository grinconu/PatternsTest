...
class PrototypeTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.loadDataAuthors()
    }
    
    private func loadDataAuthors(){
        let author = Author(id: 1, username: "Author")
        author.add(page: Page(title: "Page", contents: "1234567890 qwer", author: nil))
        authors.append(author)
        for _ in 1...5 {
            authors.append(author.copy() as! Author)
        }
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCell.EditingStyle, forRowAt indexPath: IndexPath) {
        if editingStyle == .delete {
            ...
            authors.append(authors[indexPath.row].copy() as! Author)
        }    
    }
